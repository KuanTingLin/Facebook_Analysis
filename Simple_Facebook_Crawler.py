...
while count < 3:
    ...
    for data in res_json['data']:
        ...
        while True:
            if "comments" in comment_json:
                for comment in comment_json["comments"]["data"]:
                    ...
                    comment_created_time = datetime.strptime(comment["created_time"].split("+")[0], "%Y-%m-%dT%H:%M:%S")
                    comments.append([comment_id, comment_message, comment_likes, comment_created_time])
                    comments_dicts.append({"id": comment_id, "message": comment_message, "total likes": comment_likes,
        ...
        post_story = data['story'] if 'story' in data and message != '影片上傳' else '純文字'
        post_created_time = datetime.strptime(data['created_time'].split("+")[0], '%Y-%m-%dT%H:%M:%S')
        post_datas.append([fan_page_name, post_id, message, post_likes, post_story, post_created_time, comments])
        post_data_dicts.append({"id": post_id, "message": post_message, "total likes": post_likes, "story": post_story,
